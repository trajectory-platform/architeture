...
- Для группового занятия цена фиксируется с каждого ученика и не делится между участниками. Одна booking row резервирует время преподавателя; места добавляются в неё под блокировкой вместимости. Отдельный ключ PlaceHold выводится из participant ID, а не повторяет общий ключ группы.
- До M20-09/M20-19 закрываются DEC-02/DEC-03: единицы пакета, абонемент, комиссия и плательщик; отсутствие решения блокирует соответствующий сценарий, а не переносится в реализацию.
- До M20-11 закрывается DEC-04, развёртывается Egress и проверяются поздний вход, отзыв согласия и отказ обработки.
- До M20-15 закрывается DEC-05: формула освоения темы, версии и коррекции; посещаемость сама по себе не означает освоение.
